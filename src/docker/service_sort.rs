...
/**
 * this is a common travell function
 * vec_bool , will tell whether we have to travell a vec or a map
 */
pub fn trav(
    visiting_vec: &mut HashSet<String>,
    visited_vec: &mut HashSet<String>,
    ...
    service_vec: &mut Vec<String>,
    service_map: &mut HashMap<String, DockerImageDetails>,
    services_index_map: &mut IndexMap<String, Option<Service>>,
    vec_bool: bool,
    depends_on_vec: Option<Vec<String>>,
    depends_on_map: Option<IndexMap<String, DependsCondition>>,
) -> Result<bool, CliErrors> {
    let depends_on = if vec_bool {
        depends_on_vec.ok_or_else(|| {
            CliErrors::new(String::from(format!(
                "getting error while getting dependecy of {service}"
            )))
        })?
    } else {
        depends_on_vec.ok_or_else(|| {
    ...
    // since we have already gone though it , so adding , so that if comes again , we can directly return okay and not traverse again
    visited_vec.insert(service.to_string());
